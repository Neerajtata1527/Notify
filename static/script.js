...
// PDF Export (hide button during capture)
document.getElementById('downloadPdfBtn').addEventListener('click', () => {
  const target = document.querySelector('.form-builder');
  const downloadBtn = document.getElementById('downloadPdfBtn');

  // Hide the download button before export
  downloadBtn.style.display = 'none';

  // Convert form inputs to plain text for accurate rendering
  ...
  // Now capture canvas and generate scaled PDF
  html2canvas(target, { scale: 2 }).then(canvas => {
    ...
    // Handle multi-page PDFs if content height exceeds one page
